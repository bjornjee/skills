...
| 15 Severity conflict | Shared critical/high blocking contract in doctrine and strict reviewers; the optional codegraph skill was subsequently retired at the owner's request. |
...
| 29 Delegation identity/capabilities | Structured dispatch retains a job ID; exact-job status and result checks yield the verified session ID. Discover the selected plugin and supported helper capabilities; include the TypeScript reviewer. |
...
External consumers are marketplace installation paths, the ownership manifest, and UI verdict readers. The package and ownership changes require migration; release versioning is consolidated across the stack as described below; see [ADR 001](adr/001-global-skill-contracts.md). Coordination is limited to reviewing this PR and separately authorizing global rollout.
...
| Overlapping codegraph approval categories | Superseded by retiring the codegraph skill. Strict reviewers retain the shared severity and blocking contract. |
...
## Review split of PR #84

The implementation above was preserved in seven stacked PRs: packaging, advisory hook, Codex installer, Claude rule installer, doctrine/review workflows, language examples, and UI evaluation. Historical version/proof statements describe the original PR. After #85 and #86 merged, the owner requested treating the remaining stack as one release: #87–#90 keep the main-branch version 2.0.1, and #91 carries the single patch bump to 2.0.2. This supersedes the initial per-PR version sequence. CI requires a version increase only for non-stacked PRs and the final PR of a native stack, while all PRs run manifest lockstep and the full test suite. No global rollout is part of these changes.

## PR #89 follow-up

The owner retired `codegraph-audit` in favor of direct repository inspection by coding agents and strict reviewers. The skill, companion README, and catalog entry are removed; core review and evidence requirements remain. [ADR 002](adr/002-retire-codegraph-audit.md) records the removal and rollback. Earlier references to codegraph scenario checks above describe historical verification of the retired workflow.

Claude's planning clause again explicitly requires path class, caller, frequency, and what blocks. Delegation now resolves the active Codex plugin, uses its structured background helper to retain a job ID, polls that same job, and validates the matching completed result before capturing its session ID for resumption. Ambiguous plugin selection uses direct implementation; ambiguous job results never select a replacement task.

The delegation command and result shapes were checked against Codex companion 1.0.3 with isolated synthetic job records. An explicit ID selected job A while an omitted ID selected newer job B. Background-launch verification stubbed worker creation; no live Codex worker, app-server, authentication, model call, or network operation was exercised. This proves the tested transport contracts, not live model behavior. The independent follow-up review found no remaining high or medium defects.

After this follow-up, full `make test` passes at each updated snapshot: #89 has 114 tests, #90 has 123, and #91 has 107, all with zero failures. Each count is one lower because the retired skill is no longer discovered; no tests were deleted. The final release gate still passes for 2.0.1 → 2.0.2.
