...
1. **Inventory the bundle.** The caller has passed you:
   - `rubric.md` (canonical 8-dimension rubric + preservation gate + audit gate — required)
...
2. **Read the rules in this order.**
   - `rubric.md` — the 8 fixed dimensions, preservation gate, audit gate, and 1/3/5 anchors.
   ...
   - `audit-findings.md` if present — severity-tagged findings driving dimensions 7, 8, and the audit gate.
...
6. **Apply thresholds.** Three independent inputs (canonical table in `rubric.md`, weights overridable via `weights.json`):
   ...
   - **Audit gate:** must be `PASS` or `N/A` for the overall verdict to be `PASS`. `WARN` or `FAIL` downgrades the candidate to `ITERATE`. Same binary logic as the preservation gate. P0/P1 findings = `FAIL`. Symmetric, not optional.
...
The audit is **not** a scored dimension either. It is reported via the `## Audit gate` block and gates the overall verdict independently. Dimensions 7 and 8 are downstream of audit findings; the audit gate is upstream of the overall verdict. They are related but distinct.
...
```json
{
  ...
  "scores": [
    ...
    { "dimension": "accessibility", "raw": 5, "weight": 1.0, "weighted": 5.0, "screenshot": null, "justification": "audit-findings.md: zero P0/P1; behavior-check.md keyboard traversal PASS" },
    { "dimension": "technical-quality", "raw": 5, "weight": 1.0, "weighted": 5.0, "screenshot": null, "justification": "audit-findings.md: zero P0/P1; console clean during declared flow" }
  ],
  "preservation_gate": {
    "state": "PASS",
  ...
  "audit_gate": {
    "state": "PASS",
    "evidence": "audit-findings.md fresh; zero P0/P1 findings on changed files",
    "findings_path": ".uiux-loop/audit-iter-1.md"
  },
  "critique_brief": [
...
- `scores` always lists all 8 dimensions in the order above. `N/A` dimensions use `null` for `raw`, `weighted`, `screenshot`.
...
- `audit_gate.state` is one of the four states, matching the prose block exactly. `findings_path` is the absolute or repo-relative path to the audit file the orchestrator wrote; use `null` when state is `N/A`.
...
Per-dimension scoring remains cold: you do **not** inherit prior scores, you do **not** lower a score because a prior brief item was ignored, you do **not** raise a score because a prior brief item was addressed. The 8 dimension scores are always fresh against the current screenshots (dimensions 1–6) and current `audit-findings.md` (dimensions 7–8). The diff is a separate block for the implementer; the scores are independent.

### Critique-brief item contract

Each item must:

- Name the dimension it targets (one of the 8 fixed names).
...
- **No speculation untied to a dimension.** Every critique item maps to one of the 8 dimensions. Preservation findings go in the preservation gate's evidence summary; audit findings go in the audit gate's evidence summary. If you cannot map an item, you do not raise it.
