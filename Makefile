.PHONY: help bump sync-codex sync-rules test

help: ## Show this help
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | sort | awk 'BEGIN {FS = ":.*?## "}; {printf "\033[36m%-15s\033[0m %s\n", $$1, $$2}'

bump: ## Set the plugin version in all three manifests (usage: make bump V=1.1.0)
...
test: ## Run repository tests
	@mkdir -p tmp
	TMPDIR="$(CURDIR)/tmp" node --test scripts/*.test.js
