...
2. **Plan.** State the approach and obtain agreement before nontrivial implementation (multiple affected files, competing approaches, or unclear requirements). Use the runtime-supported planning workflow; Claude-specific plan-mode tools do not apply here. Existing approval to implement a concrete proposal is agreement; do not ask again for reversible work within that scope. Break work into phases, identify risks and affected files. Red flags: "I'll plan as I go" and "I already explored, that counts as planning." Research is not an agreed plan.
